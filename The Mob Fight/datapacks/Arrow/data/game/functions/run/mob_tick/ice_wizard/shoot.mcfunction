...
execute as @e[tag=ice_magic,tag=new_temp,type=snowball] facing entity @a[limit=1,sort=nearest,gamemode=adventure,distance=..11] eyes positioned ^ ^ ^2 run function game:run/mob_tick/wizard/midder
execute as @e[tag=ice_magic,tag=new_temp,tag=!shot,type=snowball] facing entity @e[limit=1,sort=nearest,type=#game:magic_target] eyes positioned ^ ^ ^2 run function game:run/mob_tick/wizard/midder
tag @e[tag=ice_magic,tag=new_temp,type=snowball] remove new_temp
