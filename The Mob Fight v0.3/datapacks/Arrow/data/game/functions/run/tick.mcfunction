...
execute if score doArrowLandPosCheck Options matches 1.. run tag @e[type=#arrows,nbt={custom_potion_effects:[{}]},predicate=map:middle_area] add actived
execute unless score doArrowLandPosCheck Options matches 1.. run tag @e[type=#arrows,nbt={custom_potion_effects:[{}]}] add actived
execute as @e[type=#arrows,nbt={custom_potion_effects:[{}]},predicate=map:middle_area] run data merge entity @s {damage:0.25d}
...
execute as @e[type=area_effect_cloud,nbt={effects:[{id:"minecraft:dolphins_grace",show_particles:0b}]}] at @s run function game:run/creeper/cloud_self
