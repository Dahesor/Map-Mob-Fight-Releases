...
summon snowball ^ ^ ^0.5 {NoGravity:1b,Tags:["tp_magic","new_temp","ticking","snowball_parent"],LeftOwner:0b,Item:{id:"ender_eye",Count:1b,tag:{Enchantments:[{}]}},Passengers:[{Tags:["tp_magic","del",'ticking','snowball_child',"new_temp"],CustomName:'{"translate":"arrow.tp_wizard","color":"gold"}',data:{from:[I;0,0,0,0]},id:"marker"}]}

data modify entity @e[type=snowball,tag=tp_magic,limit=1,tag=new_temp] Owner set from entity @s UUID
data modify entity @e[type=marker,tag=tp_magic,limit=1,tag=new_temp] data.from set from entity @s UUID
...
tag @e[tag=tp_magic,tag=new_temp,type=marker] remove new_temp
