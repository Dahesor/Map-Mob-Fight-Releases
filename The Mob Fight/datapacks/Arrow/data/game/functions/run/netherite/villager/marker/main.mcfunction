...
data modify storage run vil.this set value {flash:0b,hp:10}
...
data merge entity @e[type=villager,tag=the_villager,limit=1,distance=..5] {Glowing:1b}

execute if score @s hpPotionCD matches 1.. run data modify storage run vil.this.flash set value 1b
...
execute store result storage run vil.this.hp byte 1 run scoreboard players get #villager_health calculator
execute if entity @s[tag=villager_red] run data modify storage run vil.red append from storage run vil.this
execute if entity @s[tag=villager_blue] run data modify storage run vil.blue append from storage run vil.this
