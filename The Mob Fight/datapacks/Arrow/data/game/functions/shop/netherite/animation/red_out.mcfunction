...
kill @e[type=armor_stand,limit=1,tag=red_vand]
execute at @e[type=marker,limit=1,tag=altar_red] run summon armor_stand ~ ~ ~ {Invisible:1b,Marker:1b, Tags:["red_vand"],ArmorItems:[{},{},{},{id:"beacon",Count:1b}],DisabledSlots:16191}
execute if data storage run map.altar.red{facing:"north"} run data merge entity @e[type=armor_stand,tag=red_vand,limit=1] {Rotation:[-180.0f,0.0f]}
execute if data storage run map.altar.red{facing:"south"} run data merge entity @e[type=armor_stand,tag=red_vand,limit=1] {Rotation:[0.0f,0.0f]}
execute if data storage run map.altar.red{facing:"west"} run data merge entity @e[type=armor_stand,tag=red_vand,limit=1] {Rotation:[90.0f,0.0f]}
execute if data storage run map.altar.red{facing:"east"} run data merge entity @e[type=armor_stand,tag=red_vand,limit=1] {Rotation:[-90.0f,0.0f]}
execute if data storage run map.altar.red{extend:1b} as @e[type=armor_stand,tag=red_vand,limit=1] at @s run tp @s ^ ^ ^1
execute if data storage run map.altar.red{drop:1b} as @e[type=armor_stand,tag=red_vand,limit=1] at @s run tp @s ^ ^-1 ^

scoreboard players set @e[type=armor_stand,tag=red_vand,limit=1,tag=!_vand] genericCD 100

execute as @e[type=armor_stand,limit=1,tag=red_vand] at @s run function game:shop/netherite/animation/spread_icons/enter

data modify storage run log.new set value {"translate":"log.vand.red","color": "red"}
