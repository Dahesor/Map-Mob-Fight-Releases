...
execute if entity @s[gamemode=creative] run scoreboard players set #h calculator 0

execute unless score #h calculator matches 0 run tellraw @s {"translate":"[商店] 材料不足！","color": "red"}
execute unless score #h calculator matches 0 at @s run playsound minecraft:entity.enderman.teleport master @s ~ ~ ~ 0.6 0.4
