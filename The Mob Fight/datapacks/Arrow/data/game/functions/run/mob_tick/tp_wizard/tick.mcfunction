...
execute if entity @a[gamemode=adventure,team=!,distance=..15,tag=!spectator] positioned ~ ~1.1 ~ facing entity @a[gamemode=adventure,team=!,limit=1,sort=nearest,tag=!spectator] eyes run function game:run/mob_tick/wizard/cast_check
