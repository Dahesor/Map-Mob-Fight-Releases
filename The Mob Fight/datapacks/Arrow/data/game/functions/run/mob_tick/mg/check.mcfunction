...
execute if entity @a[gamemode=adventure,team=!,distance=..18,tag=!spectator] positioned ~ ~1.1 ~ facing entity @a[gamemode=adventure,team=!,limit=1,sort=nearest,tag=!spectator] feet run function game:run/mob_tick/wizard/cast_check
